    ...
    public void run() {
        ...
        try {
            submissions: while (true) {
                ...
                try {
                    ...
                    if (options.getTagString() == null) {
                        ...
                        if (tagFile.canRead()) {
                            try {
                                reqEntity
                                        .addPart(SubmissionConstants.SUBMISSON_TAGS, new StringBody(readFile(tagFile)));
                            } catch (IOException e) {
                    ...
                    for (File modFile : modules) {
                        ...
                        if (modTagsFile.canRead()) {
                            try {
                                reqEntity.addPart(SubmissionConstants.MODULE_TAGS + n, new StringBody(
                                        readFile(modTagsFile)));
                            } catch (IOException e) {
                        ...
                        reqEntity.addPart(SubmissionConstants.MODULE_FILE + n, new FileBody(modFile, "text/plain",
                                "UTF-8"));
                    }
                    ...
                    for (File attFile : attachments) {
                        ...
                        if (attTagsFile.canRead()) {
                            try {
                                reqEntity.addPart(SubmissionConstants.ATTACHMENT_TAGS + n, new StringBody(
                                        readFile(attTagsFile)));
                            } catch (IOException e) {
                        ...
                        reqEntity.addPart(SubmissionConstants.ATTACHMENT_FILE + n, new FileBody(attFile,
                                "application/binary"));
                ...
                System.out.println("File '" + sbmDir.getAbsolutePath() + "' done (" + endTime + "ms) " + infiles.size()
                        + " in the queue");
